...
when defined(openbsd):
  ...
  proc arc4random_buf(buf: pointer, nbytes: csize) {.importc: "arc4random_buf",
      header: "<stdlib.h>".}

  proc arc4random_uniform(upperBound: uint32): uint32 {.
      importc: "arc4random_uniform", header: "<stdlib.h>".}
  ...
  proc RtlGenRandom(RandomBuffer: pointer, RandomBufferLength: uint64): bool {.
      cdecl, dynlib: "Advapi32.dll", importc: "SystemFunction036".}
  ...
  when defined(linux):
    ...
    proc syscall(number: clong, buf: pointer, buflen: csize,
        flags: cint): clong {.importc: "syscall", header: "<unistd.h>".}
    ...
    proc safeSyscall(buffer: pointer, size: int) =
      ...
      while mutSize > 0:
        ...
        while readNumberBytes < 0 and (lastError == OSErrorCode(EINTR) or
            lastError == OSErrorCode(EAGAIN)):
          readNumberBytes = syscall(SYS_getrandom, mutBuf, mutSize, 0)
          lastError = osLastError()
